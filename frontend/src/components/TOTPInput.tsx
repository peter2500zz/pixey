...
export function TOTPInput({ value, onChange, error, disabled, label }: TOTPInputProps) {
  ...
  const handleInput = useCallback((i: number, e: React.FormEvent<HTMLInputElement>) => {
    ...
    if (v.length > 1) {
      ...
      setTimeout(() => focus(Math.min(i + fill.length, 5)), 0)
    } else {
      update(i, v)
    }
      ...
      <div className="flex items-center gap-1.5 sm:gap-2">
        {Array.from({ length: 6 }).map((_, i) => (
          <motion.input
            ...
            onBlur={() => {}}
            disabled={disabled}
            className={['digit-input', digits[i] ? 'filled' : '', error ? 'error' : ''].filter(Boolean).join(' ')}
...
function TOTPPeriodBar() {
  ...
  const [pct, setPct] = React.useState(getPct)
  React.useEffect(() => {
    const t = setInterval(() => setPct(getPct()), 200)
  ...
  return (
    <div className="w-44 h-1 bg-surface-200 rounded-full overflow-hidden">
      <div
        ...
        style={{ width: `${pct}%`, transition: 'width 0.2s linear, background-color 0.3s' }}
      />
    </div>
  )
}
