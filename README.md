...
> **Vibe coding product** — conjured into existence by [Claude](https://claude.ai) (Anthropic).  
...
- **Zero deps at runtime** — single self-contained binary
...
| [Bun](https://bun.sh) | 1.x | Frontend build tool |
...
# Install frontend dependencies & build everything
make build
```

This produces a single `./pixey` binary that embeds the web UI.
...
`internal/web/dist/`, which Go then embeds into the binary via `//go:embed`.
...
3. Enter the 6-digit code to confirm — setup is one-time
...
displayed and copyable. Share them with whoever needs proxy access.
...
code again — useful for adding Pixey to a second device.
...
- Expired credentials keep working for `min(validity, 1 day)` before deletion
- The proxy returns `407 Proxy Auth Required` for missing/invalid credentials
